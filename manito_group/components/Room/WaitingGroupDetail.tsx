        ...
        <div className={manitoGroupStyles.card}>
          ...
          {groupData.isOwner && (
            <div className='flex gap-4 mt-4'>
              <button className={`${styles.button.blue} py-2 px-4`} onClick={handleStart}>
                시작
              </button>
              <button className={`${styles.button.green} py-2 px-4`} onClick={handleInviteBtn}>
                초대
              </button>
            </div>
          )}
...
function InvitedPeople({ name, status }: { name: string; status: InviteStatus }) {
  return (
