import { DeserializedManitoGroup, GroupStatus, PredictStatus } from '@/manito_group/model';
...
export default function EndedGroupDetail({ groupData }: { groupData: DeserializedManitoGroup }) {
  ...
  useEffect(() => {
    if (!modalOpen && endResult?.manitoResultStatus === PredictStatus.NOTSUBMIT) {
      setModalOpen(true);
    }
  }, [modalOpen, endResult]);
      ...
      { groupId: groupData.id, maniteeName },
      {
        onSuccess(result) {
          if (result.manitoResultStatus === PredictStatus.CORRECT) {
            alert('정답입니다!');
          } else if (result.manitoResultStatus === PredictStatus.INCORRECT) {
            alert(`틀렸습니다. 나를 도와준 사람은 ${result.maniteeName} 이었습니다!`);
          }
          setModalOpen(false);
        },
      }
    ...
    <div className='grid grid-cols-4 border-b-2 h-full'>
      ...
      >
        <form className={styles.form} onSubmit={handlePredictSubmit}>
          <h3 className='font-bold'>모임이 종료되었습니다. 나를 도와준 사람 이름을 맞춰보세요.</h3>
